...
export default function IndiaRwandaPage() {
  return (
    <>
      ...
      >
        <div className="flex flex-wrap gap-2">
          {CORRIDOR_FOCUS.map((focus) => (
            <span
              key={focus}
              className="inline-flex items-center rounded-full border border-white/14 px-4 py-2 text-[0.8125rem] text-ivory/75"
            >
              {focus}
            </span>
          ))}
      ...
      {/* ----------------------------------------------------- why rwanda */}
      <Section id="why-rwanda" tone="paper">
        <div className="container-page">
          ...
          <RevealGroup className="mt-14 grid gap-px overflow-hidden rounded-[var(--radius-card)] bg-hairline sm:grid-cols-2 lg:grid-cols-3">
            {WHY_RWANDA.map((item, i) => (
              <RevealItem noShift key={item.title} className="bg-paper p-8">
      ...
      {/* ------------------------------------------------------ why india */}
      <Section id="why-india" tone="ivory">
        <div className="container-page">
          <SectionHeading
            eyebrow="Why India"
      ...
      {/* ------------------------------------------------------- corridor */}
      <Section id="corridor" tone="ink" className="grain overflow-hidden">
        <div className="container-page relative">
          <div className="grid items-center gap-16 lg:grid-cols-12">
            <div className="lg:col-span-5">
              <Reveal>
                <Eyebrow tone="light">The trade corridor</Eyebrow>
              </Reveal>
              <Reveal delay={0.06}>
                <h2 className="mt-5 text-[clamp(1.9rem,4.4vw,3.25rem)] leading-[1.05] font-semibold text-ivory">
                  Delhi, Mumbai, Bengaluru{" "}
                  <span className="accent-serif text-gold-400">→ Kigali.</span>
                </h2>
              </Reveal>
              <div className="mt-6 space-y-4 text-[0.9375rem] leading-[1.7] text-ivory/62">
                <Reveal delay={0.1}>
                  <p>
      ...
      {/* --------------------------------------------------- opportunities */}
      <Section id="opportunities" tone="paper">
        <div className="container-page">
          <SectionHeading
          ...
          <RevealGroup className="mt-14 grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
            {OPPORTUNITIES.map((item, i) => (
              <RevealItem key={item.title}>
                <article className="group relative h-full overflow-hidden rounded-[var(--radius-card)] border border-hairline bg-paper p-7 transition-[border-color,transform] duration-500 ease-[var(--ease-out-quint)] hover:-translate-y-1 hover:border-navy-800/25">
                  <div
